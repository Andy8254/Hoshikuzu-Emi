...
## 설명
플레이어는 Check in 버튼을 눌러 참가 신청을 확정할 수 있습니다.
이때, 봇은 참가 신청 때 등록된 유저네임을 자동으로 사용합니다.
## 참고
`closes_at`은 일반 체크인이 마감되는 Unix 타임스탬프입니다.
`grace_time`은 `closes_at` 이후에 허용되는 추가 지각 체크인 시간이며, 단위는 초입니다. 위 옵션을 기본 체크인 시간을 설정하는 데 사용하지 말아주세요.
